...
public class ImpactSearchController implements Serializable {
	...
	private int currentOrTarget = 0; // represents if viewing LIST is from
										// current table or target table

	private ListNotesFormVM notesFormModel = new ListNotesFormVM(); // "Informative
																	// Notes"
																	// tab model
	...
	public void filterRelationsByScope(AjaxBehaviorEvent event) {
		...
		IARelationsTreeHelper treeHelper = new IARelationsTreeHelper(cmqBaseCurrentService, smqBaseCurrentService,
				meddraDictCurrentService, cmqRelationCurrentService, cmqBaseTargetService, smqBaseTargetService,
				meddraDictTargetService, cmqRelationTargetService);
		treeHelper.onNodeExpandTargetTableScope(targetTableRootTreeNode, null, scopeFilter);
					...
					if (entity instanceof CmqBaseTarget) {
						...
						for (TreeNode treeNode : nodesList) {
							...
							if ((null != hierarchyNode) && !hierarchyNode.isDummyNode()) {
								// first check if this node is already added ot
								// relations tree
								...
								if (CollectionUtils.isNotEmpty(existingRelationsTreeNodes)) {
									for (TreeNode existingRelationsTreeNode : existingRelationsTreeNodes) {
										HierarchyNode existingHierarchyNode = (HierarchyNode) existingRelationsTreeNode
												.getData();
										if (hierarchyNode.getCode().equalsIgnoreCase(existingHierarchyNode.getCode())
												&& hierarchyNode.getLevel()
														.equalsIgnoreCase(existingHierarchyNode.getLevel())) {
											exists = true;
								...
								if (!exists) {
									...
									if (!nodesList.contains(parentNode)) {
										...
										relationsHierarchyNode.setRowStyleClass("green-colored");// mark
																									// this
																									// row
																									// as
																									// green
										TreeNode relationsTreeNode = new DefaultTreeNode(relationsHierarchyNode,
												parentTreeNode);
										...
										if (relationsHierarchyNodeEntity instanceof SMQReverseHierarchySearchDto) {
											Long ptCode = ((SMQReverseHierarchySearchDto) relationsHierarchyNodeEntity)
													.getSmqCode();
						...
						if (CollectionUtils.isNotEmpty(existingNodeTerms)) {
							...
							FacesContext.getCurrentInstance().addMessage(null, new FacesMessage(
									FacesMessage.SEVERITY_INFO, "Selected relations added sucessfully.", ""));
						}
					} else {
					...
					if (CollectionUtils.isNotEmpty(childTreeNodes)) {
						for (TreeNode childTreeNode : childTreeNodes) {
							...
							if (null != hierarchyNode) {
								...
								if (childEntity instanceof CmqBaseTarget) {
									...
									// TODO change code here for parent child
									// relationship
									...
									if (childEntity instanceof MeddraDictHierarchySearchDto) {
										...
										matchingMap = this.checkIfMeddraRelationExists(existingRelations, level,
												hierarchyNode);
										matchFound = (boolean) matchingMap.get("MATCH_FOUND");
										updateNeeded = (boolean) matchingMap.get("UPDATE_NEEDED");
										if (updateNeeded) {
											cmqRelation = (CmqRelationTarget) matchingMap
													.get("TARGET_CMQ_RELATION_FOR_UPDATE");
										} else {
										...
										if ("PT".equalsIgnoreCase(hierarchyNode.getLevel())) {
											...
											matchingMap = this.checkIfReverseMeddraRelationExists(existingRelations,
													code, hierarchyNode);
										...
										matchingMap = this.checkIfSmqBaseOrSmqRelationExists(existingRelations,
												smqBase.getSmqCode(), null, hierarchyNode);
										matchFound = (boolean) matchingMap.get("MATCH_FOUND");
										updateNeeded = (boolean) matchingMap.get("UPDATE_NEEDED");
										if (updateNeeded) {
											cmqRelation = (CmqRelationTarget) matchingMap
										...
										matchingMap = this.checkIfSmqBaseOrSmqRelationExists(existingRelations,
												smqRelation.getSmqCode(), smqRelation.getPtCode(), hierarchyNode);
										matchFound = (boolean) matchingMap.get("MATCH_FOUND");
										updateNeeded = (boolean) matchingMap.get("UPDATE_NEEDED");
										if (updateNeeded) {
											cmqRelation = (CmqRelationTarget) matchingMap
											...
											cmqRelation.setCmqCode(cmqBaseTarget.getCmqCode());
											cmqRelation.setCmqId(cmqBaseTarget.getId());
											// we set both smqcode and pt code
											// to show that this is an smq
											// relation
										...
										matchingMap = this.checkIfSmqBaseOrSmqRelationExists(existingRelations,
												smqReverseHierarchySearchDto.getSmqCode(),
						...
						if (!cmqRelationsList.isEmpty() || !cmqBaseChildrenList.isEmpty()) {
							try {
								...
								LOG.error(
										"Exception occurred while updated the list of CmqRelations for CMQ base target code "
						 ...
						 * CmqBaseTarget parentCmq =
						 * this.cmqBaseTargetService.findByCode(cmqBaseTarget.
						 * getCmqParentCode());
						 * if("NON-IMPACTED".equalsIgnoreCase(parentCmq.
						 * getImpactType())) { parentCmq.setImpactType("ICC");
						 * parentCmq.setCmqState("PENDING IA");
						 * parentCmq.setCmqStatus("P");
						 * impactedCmqsList.add(parentCmq); }
						 * List<CmqBaseTarget> childrenOfParentCmq =
						 * this.cmqBaseTargetService.findChildCmqsByParentCode(
						 * parentCmq.getCmqCode()); for
						 * (ListIterator<CmqBaseTarget> li =
						 ...
						 * cmqBaseTarget.getCmqCode().longValue()) {
						 * li.remove();//Remove this cmq as we are already
						 * dealing with it. } else
						 * if("NON-IMPACTED".equalsIgnoreCase(childOfParentCmq.
						 * getImpactType())) {
	...
	private Map<String, Object> checkIfMeddraRelationExists(List<CmqRelationTarget> existingRelation, String matchKey,
		...
		MeddraDictHierarchySearchDto meddraDictHierarchySearchDto = (MeddraDictHierarchySearchDto) hierarchyNode
				.getEntity();
	...
	private Map<String, Object> checkIfReverseMeddraRelationExists(List<CmqRelationTarget> existingRelations, Long code,
			HierarchyNode hierarchyNode) {
		Map<String, Object> matchingMap = new HashMap<String, Object>(3);
		matchingMap.put("MATCH_FOUND", false);
		matchingMap.put("UPDATE_NEEDED", false);
		...
		for (CmqRelationTarget cmqRelationTarget : existingRelations) {
			// check for PT or LLT
			if ((null != cmqRelationTarget.getPtCode())
					&& (cmqRelationTarget.getPtCode().longValue() == code.longValue())) {
				matchingMap.put("MATCH_FOUND", true);
			} else if ((null != cmqRelationTarget.getLltCode())
					&& (cmqRelationTarget.getLltCode().longValue() == code.longValue())) {
				matchingMap.put("MATCH_FOUND", true);
			} else if ((null != cmqRelationTarget.getHlgtCode())
					&& (cmqRelationTarget.getHlgtCode().longValue() == code.longValue())) {
				matchingMap.put("MATCH_FOUND", true);
			} else if ((null != cmqRelationTarget.getHltCode())
					&& (cmqRelationTarget.getHltCode().longValue() == code.longValue())) {
				matchingMap.put("MATCH_FOUND", true);
			} else if ((null != cmqRelationTarget.getSocCode())
					&& (cmqRelationTarget.getSocCode().longValue() == code.longValue())) {
				matchingMap.put("MATCH_FOUND", true);
			}
	...
	private Map<String, Object> checkIfSmqBaseOrSmqRelationExists(List<CmqRelationTarget> existingRelations,
			Long smqCode, Integer ptCode, HierarchyNode hierarchyNode) {
		Map<String, Object> matchingMap = new HashMap<String, Object>(3);
		matchingMap.put("MATCH_FOUND", false);
		matchingMap.put("UPDATE_NEEDED", false);
		matchingMap.put("TARGET_CMQ_RELATION_FOR_UPDATE", null);
		...
		for (CmqRelationTarget cmqRelationTarget : existingRelations) {
			if ((null != ptCode) && (null != cmqRelationTarget.getSmqCode())
					&& (cmqRelationTarget.getSmqCode().longValue() == smqCode.longValue())
					&& (null != cmqRelationTarget.getPtCode())
					&& (cmqRelationTarget.getPtCode().longValue() == ptCode.longValue())) {
				// its an smqrelation and not an smqbase
				matchingMap.put("MATCH_FOUND", true);
			} else if ((null != cmqRelationTarget.getSmqCode())
					&& (cmqRelationTarget.getSmqCode().longValue() == smqCode.longValue())) {
				// its an smqbase
				matchingMap.put("MATCH_FOUND", true);
			}
	...
	private boolean checkIfExistingRelationNeedsUpdate(CmqRelationTarget cmqRelationTarget,
			HierarchyNode hierarchyNode) {
		boolean needsUpdate = false;
		// first match scope
		if (StringUtils.isBlank(hierarchyNode.getScope()) && !StringUtils.isBlank(cmqRelationTarget.getTermScope())) {
			needsUpdate = true;
		} else if (!StringUtils.isBlank(hierarchyNode.getScope())
				&& StringUtils.isBlank(cmqRelationTarget.getTermScope())) {
			needsUpdate = true;
		} else if (!StringUtils.isBlank(hierarchyNode.getScope())
				&& !StringUtils.isBlank(cmqRelationTarget.getTermScope())
		...
		// now so category
		if (StringUtils.isBlank(hierarchyNode.getCategory())
				&& !StringUtils.isBlank(cmqRelationTarget.getTermCategory())) {
			needsUpdate = true;
		} else if (!StringUtils.isBlank(hierarchyNode.getCategory())
				&& StringUtils.isBlank(cmqRelationTarget.getTermCategory())) {
			needsUpdate = true;
		} else if (!StringUtils.isBlank(hierarchyNode.getCategory())
				&& !StringUtils.isBlank(cmqRelationTarget.getTermCategory())
	...
	public String onIaWizardFlowProcess(FlowEvent event) {
		...
		// if (selectedImpactedCmqList != null || selectedImpactedSmqList !=
		// null || selectedNotImpactedCmqList != null ||
		...
		if ("impact".equalsIgnoreCase(event.getOldStep()) && (this.currentTableRootTreeNode.getChildCount() == 1)
				&& targetRelationsUpdated) {
		...
		if ("impact".equalsIgnoreCase(event.getNewStep())) {
			...
			// if the target tab is "notes", allow it only when there is a valid
			// CmqBase* or SmqBase* object selected from current or target lists
			Object d = null;
			...
			// if the target tab is "details", allow it only when there is a
			// valid CmqBase* or SmqBase* object selected from current or target
			// lists
			Object d = null;
			if (currentOrTarget == SELECTED_CURRENT_LIST && currentTableSelection != null) {
				HierarchyNode hn = (HierarchyNode) currentTableSelection.getData();
			...
			if (d instanceof CmqBase190) {
				...
				FacesMessage msg = new FacesMessage(FacesMessage.SEVERITY_WARN,
						"Details tab is not accessible for SMQs", "");
				FacesContext.getCurrentInstance().addMessage(null, msg);
				RequestContext.getCurrentInstance().update("impactAssessment:messages");
				// if (event.)
				return "notes";
	...
	public String onIaVersionWizardFlowProcess(FlowEvent event) {
		...
		/*
		 * if("impact".equalsIgnoreCase(event.getNewStep())) { // if the target
		 * tab is "Impact Assessment", allow it always if(unsavedRedirect)
		 * iaWizardNextStep = event.getNewStep(); else nextStep =
		 * event.getNewStep(); } else
		 */ if ("notes".equalsIgnoreCase(event.getNewStep())) {
			// if the target tab is "notes", allow it only when there is a valid
			// CmqBase* or SmqBase* object selected from current or target lists
			Object d = null;
			if (currentOrTarget == SELECTED_CURRENT_LIST && currentTableSelection != null) {
	...
	public void saveInformativeNotes() {
		...
		try {
			...
			FacesContext.getCurrentInstance().addMessage(null,
					new FacesMessage(FacesMessage.SEVERITY_INFO, "Successfully saved notes", ""));
		} catch (CqtServiceException e) {
			FacesContext.getCurrentInstance().addMessage(null,
					new FacesMessage(FacesMessage.SEVERITY_ERROR, "Failed to save notes", "Error: " + e.getMessage()));
		}
	}
	...
	public void saveDetails() {
		...
		try {
			...
			FacesContext.getCurrentInstance().addMessage(null,
					new FacesMessage(FacesMessage.SEVERITY_INFO, "Successfully saved details", ""));
		} catch (CqtServiceException e) {
			FacesContext.getCurrentInstance().addMessage(null, new FacesMessage(FacesMessage.SEVERITY_ERROR,
					"Failed to save details", "Error: " + e.getMessage()));
		}
	}
	...
	public void cancelDetails() {
		...
		FacesContext.getCurrentInstance().addMessage(null,
				new FacesMessage(FacesMessage.SEVERITY_INFO, "Canceled the details form", ""));
	}
	...
	public void loadNewPts() {
		...
		DataTable dataTable = (DataTable) FacesContext.getCurrentInstance().getViewRoot()
				.findComponent("impactAssessment:newPtResultList");
		dataTable.reset();
		dataTable.loadLazyData();
	}

	public void addSelectedNewPtsToTargetRelation() {
		IARelationsTreeHelper treeHelper = new IARelationsTreeHelper(cmqBaseTargetService, smqBaseTargetService,
				meddraDictTargetService, cmqRelationTargetService);
		if ((this.selectedNewPtLists != null) && (this.currentTableRootTreeNode.getChildCount() == 1)) {
			...
			for (MeddraDictHierarchySearchDto meddraDictHierarchySearchDto : selectedNewPtLists) {
				...
				if (!exists) {
					...
					Long countOfChildren = this.meddraDictTargetService.findChildrenCountByParentCode("LLT_", "PT_",
							dtoCode);
			...
			FacesMessage msg = new FacesMessage(FacesMessage.SEVERITY_INFO,
					"No New PTs selected for addition to target table.", "");
			FacesContext ctx = FacesContext.getCurrentInstance();
			ctx.addMessage(null, msg);
		}
	}
	...
	/**
	 * Returns the CMQ/SMQ relation impact type message (Hover text on relations
	 * hierarchy row/code column):
	 * 
	 * @param node
	 * @return
	 */
			...
			if (null != entity) {
				...
				if (entity instanceof CmqBaseTarget) {
					...
					try {
						...
						FacesMessage message = new FacesMessage(FacesMessage.SEVERITY_ERROR, "Loading....",
								"Error while removing cmq_parent_code value from cmq_id " + cmqEntity.getId()
										+ " Error is:" + exception);
						FacesContext.getCurrentInstance().addMessage(null, message);
					}
				} else {
					List<CmqRelationTarget> existingRelation = this.cmqRelationTargetService
							.findByCmqCode(parentCmqCode);
					if ((null != existingRelation) && (existingRelation.size() > 0)) {
						...
						if (entity instanceof MeddraDictHierarchySearchDto) {
							...
							MeddraDictReverseHierarchySearchDto reverseSearchDto = (MeddraDictReverseHierarchySearchDto) hierarchyNode
									.getEntity();
							String level = hierarchyNode.getLevel();
							for (CmqRelationTarget cmqRelationTarget : existingRelation) {
							...
							} // end of for (CmqRelation190 cmqRelation190 :
								// existingRelation)
						} else if (entity instanceof SmqBaseTarget) {
							SmqBaseTarget smqBase = (SmqBaseTarget) entity;
							for (CmqRelationTarget cmqRelationTarget : existingRelation) {
								if ((null != cmqRelationTarget.getSmqCode()) && (cmqRelationTarget.getSmqCode()
										.longValue() == smqBase.getSmqCode().longValue())) {
									matchFound = true;
									cmqRelationIdToDelete = cmqRelationTarget.getId();
									break;
								}
							...
							for (CmqRelationTarget cmqRelationTarget : existingRelation) {
								if ((null != cmqRelationTarget.getSmqCode()) && (cmqRelationTarget.getSmqCode()
										.longValue() == smqRelation.getSmqCode().longValue())) {
									matchFound = true;
									cmqRelationIdToDelete = cmqRelationTarget.getId();
									break;
								}
						...
						if (matchFound && (cmqRelationIdToDelete != null)) {
							try {
								this.cmqRelationTargetService.remove(cmqRelationIdToDelete,
										this.authService.getUserCn(), this.authService.getUserGivenName(),
								...
								String exception = CmqUtils.getExceptionMessageChain(e);
								FacesMessage message = new FacesMessage(FacesMessage.SEVERITY_ERROR, "Loading....",
										"Failed to delete the relation for cmqRelationId. Error is: " + exception);
								FacesContext.getCurrentInstance().addMessage(null, message);
							}
						}
					}
				...
				if (isDeletSuccessful) {
					try {
						this.cmqBaseTargetService.update(cmqBaseTarget, this.authService.getUserCn(),
								...
								this.authService.getCombinedMappedGroupMembershipAsString());
						FacesContext.getCurrentInstance().addMessage(null,
								new FacesMessage(FacesMessage.SEVERITY_INFO, "Relation deleted successfully.", ""));
					} catch (CqtServiceException e) {
						LOG.error("Error while making the cmq target as IMPACTED.", e);
						String exception = CmqUtils.getExceptionMessageChain(e);
						FacesContext.getCurrentInstance().addMessage(null,
								new FacesMessage(FacesMessage.SEVERITY_ERROR, "Loading....",
		...
		try {
			...
			if (this.isNonImpactedCmqSelected)
				cmqBaseTargetService.update(selectedNotImpactedCmqList, this.authService.getUserCn(),
						...
						this.authService.getCombinedMappedGroupMembershipAsString());
			FacesContext.getCurrentInstance().addMessage(null,
					new FacesMessage(FacesMessage.SEVERITY_INFO, "Workflow state has been updated", ""));
		} catch (CqtServiceException e) {
			e.printStackTrace();
			FacesContext.getCurrentInstance().addMessage(null, new FacesMessage(FacesMessage.SEVERITY_ERROR,
	...
	private void setCmqBaseAsImpacted(CmqBaseTarget cmqBaseTarget) {
		if (null != cmqBaseTarget) {
			...
			List<CmqBaseTarget> children = this.cmqBaseTargetService
					.findChildCmqsByParentCode(cmqBaseTarget.getCmqCode());
			if (null != children) {
			...
			try {
				...
				LOG.error(
						"Exception occurred while updated the cmq, its parent and other children of the parent for base CMQ code "
								+ cmqBaseTarget.getCmqCode(),
						e);

				FacesContext.getCurrentInstance().addMessage(null,
						new FacesMessage(FacesMessage.SEVERITY_ERROR,
								"An error occurred while updated the impac type for CMQ base code "
										+ cmqBaseTarget.getCmqCode(),
								"Error:" + e.getMessage()));
			}
		}
	...
	private class NewPtSearchLazyDataModel extends LazyDataModel<MeddraDictHierarchySearchDto> {
		...
		public List<MeddraDictHierarchySearchDto> load(int first, int pageSize, List<SortMeta> multiSortMeta,
			...
			LOG.info("Loading more new pt list MeddraDictHierarchySearchDto starting from " + first
	...
	public List<RefConfigCodeList> getLevelListForIA() {
		List<RefConfigCodeList> levels = refCodeListService
				.findByConfigType(CqtConstants.CODE_LIST_TYPE_DICTIONARY_LEVELS, OrderBy.ASC);
		RefConfigCodeList levelToRemove = refCodeListService
				.findByConfigTypeAndInternalCode(CqtConstants.CODE_LIST_TYPE_DICTIONARY_LEVELS, "NC-LLT");
	...
	public boolean isDemoteEnabled() {
		CmqBaseTarget t = (this.isImpactedCmqSelected ? this.selectedImpactedCmqList : this.selectedNotImpactedCmqList);
		return (currentOrTarget == SELECTED_TARGET_LIST && t != null
				&& ("REVIEWED IA".equals(t.getCmqState()) || "APPROVED IA".equals(t.getCmqState())));
	}
	...
	public boolean isNotesFormReadonly() {
		...
		if (authService.getGroupMembershipHeader() != null
			...
			if (detailsFormModel.getStatus().equals(CmqBaseTarget.CMQ_STATUS_VALUE_ACTIVE)
					|| (detailsFormModel.getDesignee() != null
							&& detailsFormModel.getDesignee().equals(authService.getUserCn()))
					|| (detailsFormModel.getDesigneeTwo() != null
							&& detailsFormModel.getDesigneeTwo().equals(authService.getUserCn()))
					|| (detailsFormModel.getDesigneeThree() != null
							&& detailsFormModel.getDesigneeThree().equals(authService.getUserCn()))) {
	...
	/*
	 * public boolean isChangeOccur() { return changeOccur; }
	 * 
	 * public void setChangeOccur(boolean changeOccur) { this.changeOccur =
	 * changeOccur; }
	 */
	...
	public boolean isHierachySearchButtonEnabled() {
		return ((selectedImpactedCmqList != null) || (selectedNotImpactedCmqList != null)
				|| (selectedImpactedSmqList != null) || (selectedNotImpactedSmqList != null));
	}
	...
	public boolean isReadOnlyIA() {
		return ((selectedImpactedCmqList != null && selectedImpactedCmqList.getCmqState() != null
				&& (selectedImpactedCmqList.getCmqState().equals("APPROVED IA")
						|| selectedImpactedCmqList.getCmqState().equals("PUBLISHED IA")))
				|| (selectedNotImpactedCmqList != null && selectedNotImpactedCmqList.getCmqState() != null
						&& (selectedNotImpactedCmqList.getCmqState().equals("PUBLISHED IA")
								|| selectedNotImpactedCmqList.getCmqState().equals("APPROVED IA"))));
		// return readOnlyIA;
	}
